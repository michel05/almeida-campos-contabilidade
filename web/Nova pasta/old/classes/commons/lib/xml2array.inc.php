 ...
 */
function & xmlFileToArray($xml, $includeTopTag = false, $lowerCaseTags = true) {
                              ...
                              & $levels, & $prevTag, & $multipleData, & $xml)
{
    ...
    $currTag = '';
    $index = NULL;
    if(!in_array($currTag, $multipleData)) {
    	$multipleData[$currTag] = array();
    }
    // don't include top tag
    ...
    if (in_array("multiple", $multipleData[$currTag]))
    {
