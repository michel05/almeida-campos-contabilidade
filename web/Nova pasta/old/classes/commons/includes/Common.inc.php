...
//check if we are in style preview mode:
} else if ($modeString === "style_preview" && count($requestParts) > 4) {
    define('_SYSTEM_MODE', 'STYLE_PREVIEW');
    define('_SYSTEM_STYLE', rawurldecode($requestParts[3]));
    define('_MOBILE', false);
    define('_FACEBOOK', false);
} else {
...
if (!defined('_SKIP_DEFINITION_INCLUDES')) {
    ...
    if (_SYSTEM_MODE == 'STYLE_PREVIEW') {
        ob_start();

        if (!(include_once(getDefinitionsPath($modeString . '/' . rawurlencode(_SYSTEM_STYLE))))) {
            ob_end_clean();
            fourOhFour();
        }
        ob_end_clean();

        // Etag different for each previewStyle
        $systemProperties['system']['page']['etag'] = md5(_SYSTEM_STYLE . $systemProperties['system']['page']['etag']);
    } else if (_SYSTEM_MODE != 'RUN') {
        ob_start();

        if (!(include_once(getDefinitionsPath($modeString)))) {
            ob_end_clean();
            fourOhFour();
        }
        ob_end_clean();
...
function fourOhFour() {
    ...
    $style = "background-color: #f3f3f3;border: 1px solid #ccc;width:400px;margin-left:auto;margin-right:auto;margin-top:100px;padding:10px;font-family:verdana;";

    if (file_exists(_ENV_PROJECT_BASE . DIRECTORY_SEPARATOR . 'domain.info')) {
        $home = trim(file_get_contents(_ENV_PROJECT_BASE . DIRECTORY_SEPARATOR . 'domain.info'));
        echo "<div style='" . $style . "'>404. Sorry, we can't seem to find that page, click <a href='http://" . $home . "'>here</a> to return to the homepage</div>";
    } else {
        echo "<div style='" . $style . "'>404. Sorry, we can't seem to find that page, click <a href='/'>here</a> to return to the homepage</div>";
    }
...
function checkNotModified() {
    ...
    $noneMatch = getNoneMatch();
    $lastModified = getLastModified();
    $etag = getEtag();

    if (($modifiedSince || $noneMatch) &&
            (!$modifiedSince || ($lastModified && $lastModified <= $modifiedSince)) &&
            (!$noneMatch || ($etag && $etag == $noneMatch))) {
...
function checkPreviewAuthorization() {
    ...
    if (_SYSTEM_MODE != 'DESIGN' && _SYSTEM_MODE != 'RUN' && !_DEBUG) {
        ...
        $previewNotAllowedURL = $systemProperties['system']['previewNotAllowedURL'];
...
function getLastModified() {
    ...
    return $systemProperties['system']['page']['lastModified'];
}
...
function getEtag() {
    ...
    return $systemProperties['system']['page']['etag'];
}
...
function displayMobile() {
    ...
    if ($systemProperties['properties']['mobile']['enabled'] == "true" && isSilver()) {
